<?
	...
	switch($npcStep){
		...
		case 2:
			$response['question'] = '
			Поездка из <b>Хоэнн</b> в <b>Канто</b> будет стоить <b><i>350.000 монет</i></b>. Вы будете добираться туда <b>1 час 30 минут</b>.
			';
			$response['answer'] = array(
				3 => "Я хочу в Канто"
			);
		...
		case 3:
			if(!npc_time_check($npcId)){
				if(item_isset(1,350000)){
					...
					quest_update(1000,2);
					minus_item(1,350000);
					$response['actionQuestMinus'] = '<img src="/img/world/items/little/1.png" class="item"> Монета (350000 шт.)';
				}else{
		...
		break;
		case 5:
			if(quest_step(1000,2) && !npc_time_check($npcId)){
				//update_ach(14,1);
		...
		default:
			...
			Поездка из <b>Хоэнн</b> в <b>Канто</b> будет стоить <b><i>350.000 монет</i></b>
			';
			$response['answer'] = array(
